...
<head>
    ...
    <title>@yield('title', 'اطمینان')</title>
    <meta name="description" content="@yield('meta_description', 'فروشگاه تخصصی لوازم یدکی خودرو')">
...
<div 
    x-data="{
        ...
        doSearch() {
            ...
            fetch('{{ url('/live-search') }}?q=' + this.query)
                .then(res => res.json())
                .then(data => {
                    this.results = data;
                    this.open = true;
...
<nav 
    ...
    <a href="{{ route('shop.index') }}" class="hover:text-brand transition">
        فروشگاه
    </a>
...
<div class="relative"
    ...
    <div x-show="openMenu === 'categories'"
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-10">
            ...
            @foreach(\App\Models\Category::whereNull('parent_id')->with('children')->get() as $parent)
...
<div 
    ...
    <div class="px-6 py-6 space-y-6"
        ...
        <!-- ✅ Home -->
        <a href="{{ route('home') }}" class="block text-lg">
            صفحه اصلی
        </a>

        <!-- ✅ Shop -->
        <a href="{{ route('shop.index') }}" class="block text-lg">
            فروشگاه
        </a>
